...
class Model(ModelDesc):
    ...
    def _build_graph(self, input_vars):
        ...
        def add_layer(name, l, store_con, store_ker, out_channel, mask_scope, reuse, i=None):
            ...
            with tf.variable_scope(name) as scope:
                ...
                if not reuse:
                    m_con, m_ker = get_mask(c, in_channel, in_channel // out_channel, mask_scope, reuse, i)
        ...
        def add_bottleneck(name, l, store_con, store_ker, out_channel, mask_scope, reuse, increase_dim=False, pool=False):
            in_shape = l.get_shape().as_list()
            in_channel = in_shape[3]
            with tf.variable_scope(name) as scope:
                c = l
                ...
                for i in range(LAYER_NUM):
                    c, store_con, store_ker = add_layer('dense_layer.{}'.format(i), c, store_con, store_ker, out_channel, mask_scope, reuse, i)
...
def get_config():
    log_dir = 'train_log/cifar10-bottleneck-jointrain-N%s-L%s-K%s' % (str(BOTTLE_NUM), str(LAYER_NUM), str(KERNEL))
