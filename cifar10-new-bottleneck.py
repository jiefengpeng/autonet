    ...
    def _build_graph(self, input_vars):
        ...
        def add_bottleneck(name, l, mask, out_channel, increase_dim=False, pool=False):
            ...
            with tf.variable_scope(name) as scope:
                ...
                if increase_dim and pool: short_cut = AvgPooling('residual_pool', short_cut, 2)
...
def get_config():
    log_dir = 'train_log/cifar10-bottleneck-hand-N%s-L%s-K%s-B%s' % (str(BOTTLE_NUM), str(LAYER_NUM), str(KERNEL), '4')
