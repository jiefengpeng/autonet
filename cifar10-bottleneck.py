...
class Model(ModelDesc):
    ...
    def _build_graph(self, input_vars):
        ...
        def get_mask(l, in_channel, channel, mask_scope, reuse=True):
		    ...
		    # auto concat
                    ...
                    update_m_con = moving_averages.assign_moving_average(moving_m_con, m_con, 0.9,
		    ...
		    # auto kernel
                    ...
                    #update_m_ker = moving_averages.assign_moving_average(moving_m_ker, m_ker, 0.9,
        ...
        def add_layer(name, l, out_channel, mask_scope, reuse):
            ...
            with tf.variable_scope(name) as scope:
                ...
                m_con, m_ker = get_mask(c, in_channel, in_channel // out_channel, mask_scope, reuse)
        ...
        def add_bottleneck(name, l, out_channel, mask_scope, reuse, increase_dim=False, last=False):
            in_shape = l.get_shape().as_list()
            in_channel = in_shape[3]
            with tf.variable_scope(name) as scope:

                for i in range(LAYER_NUM):
                    l = add_layer('dense_layer.{}'.format(i), l, out_channel, mask_scope, reuse)

                #if increase_dim: out_channel *= 2
                l = add_transition('transition', l, out_channel, pool=increase_dim if not last else False)
            return l
        ...
        def dense_net(name):
            ...
            with tf.variable_scope('block1') as scope:
                ...
                with tf.variable_scope('dense_bottleneck.0'):
                    for i in range(LAYER_NUM):
                        l = add_layer('dense_layer.{}'.format(i), l, 16, MASK_scope, reuse=False)
                    l = add_transition('transition', l, 32)

            #    for i in range(1, self.N):
            #        l = add_bottleneck('dense_bottleneck.{}'.format(i), l, 16, MASK_scope, reuse=True, increase_dim=True if i == (self.N-1) else False)

            #with tf.variable_scope('block2') as scope:

            #    for i in range(self.N):
            #        l = add_bottleneck('dense_bottleneck.{}'.format(i), l, 32, MASK_scope, reuse=True, increase_dim=True if i == (self.N-1) else False)

            #with tf.variable_scope('block3') as scope:

            #    for i in range(self.N):
            #        l = add_bottleneck('dense_bottleneck.{}'.format(i), l, 64, MASK_scope, reuse=True, increase_dim=True if i == (self.N-1) else False, last=True)

            #with tf.variable_scope('block4') as scope:

            #    for i in range(self.N):
            #        l = add_bottleneck('dense_bottleneck.{}'.format(i), l, 128, MASK_scope, reuse=True,increase_dim=True if i == (self.N-1) else False)
...
def get_config():
    log_dir = 'train_log/cifar10-bottleneck-N%s-L%s' % (str(BOTTLE_NUM), str(LAYER_NUM))
    ...
    callbacks=[
        ...
        TensorPrinter(['block1/dense_bottleneck.0/dense_layer.{}/mask_con/EMA:0'.format(i) for i in xrange(10)]) #+ ['block2/dense_bottleneck.0/dense_layer.{}/mask_con/EMA:0'.format(j) for j in xrange(10)] + ['block3/dense_bottleneck.0/dense_layer.{}/mask_con/EMA:0'.format(k) for k in xrange(10)])
    ]
    if nr_tower == 1:
